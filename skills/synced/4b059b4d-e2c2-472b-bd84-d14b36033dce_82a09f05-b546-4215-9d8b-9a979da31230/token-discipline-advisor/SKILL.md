...
description: "Evaluate whether a proposed token-saving rule (for CLAUDE.md, a skill, or an agent instruction) would actually help or would cost task quality, by checking it against real usage evidence in the user's agentsview database (~/.agentsview/sessions.db) instead of trusting the rule's stated rationale. Use when the user asks to add, tighten, or judge any instruction aimed at cutting tokens/cost (e.g. truncating tool output, limiting reads, capping search, capping images) — before adopting it into CLAUDE.md or a skill."
---
...
query with Python's `sqlite3` module (`sqlite3` CLI is often not installed in
the sandbox).
